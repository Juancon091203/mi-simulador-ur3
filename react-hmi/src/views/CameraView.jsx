    ...
    <div className="camera-view-container">
      ...
      <div className="camera-config-panel">
        <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border-glass)', padding: '20px', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <h3 style={{ fontSize: '0.9rem', marginBottom: '5px', fontWeight: 'bold' }}>{t('camera_2d')}</h3>
          <div style={styles.section}>
            <label style={styles.label}>{t('auto_exposure')}</label>
          ...
          <div style={styles.section}>
            <label style={styles.label}>{t('exposure_time')}</label>
            <input type="range" min="1" max="40" defaultValue="10" />
          </div>
          <div style={styles.section}>
            <label style={styles.label}>{t('gain')}</label>
            <input type="range" min="1" max="128" defaultValue="64" />
          </div>
        </div>

        <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border-glass)', padding: '20px', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <h3 style={{ fontSize: '0.9rem', marginBottom: '5px', fontWeight: 'bold' }}>{t('vibration_tolerances')}</h3>
          <CameraViewer
